...
**The spatial-cluster null relies on a proximity proxy for independence.** The cluster null treats single-linkage spatial clusters as the unit of independence, with the linkage threshold swept from 25 to 100 km. Spatial proximity is only a proxy for shared architectural tradition: the database carries no culture or date labels, so genuinely independent structures that happen to lie close together are merged, while dispersed members of one tradition are not. The verdict — no pole distinguishable from chance — is stable across the swept thresholds, but a culture-resolved independence analysis was not possible with the available metadata.
