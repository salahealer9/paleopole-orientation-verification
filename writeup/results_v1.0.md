...
**Version note (v3).** This is version 3 of this report. Version 2 (deposited 1 June 2026) reported a preliminary positive result: apparent clustering at Poles II and III that survived the pre-registered and conditional null models. Post-publication review identified two independence controls that version had not applied — a latitude look-elsewhere correction and a control for spatial non-independence of structures. Three additional analyses were run to apply them (scripts `07`–`10` in the repository: latitude look-elsewhere, finer-block sensitivity, and a spatial-cluster null). They overturn the preliminary positive; the corrected conclusion is reported here. Version 2 remains available as a prior version of this Zenodo record; the pre-registered and v2 analyses themselves are unchanged. The new analyses are presented in §3.8 and their relationship to v2 is set out in Appendix B. Per pre-registration §12 point 3, all three are labelled exploratory.

Version 3.1 (this version) responds to the data owner's 5 June and 8 June 2026 follow-up letters (Appendices D and E). It foregrounds the spatial-cluster null's central caveat beside the result (§3.8.3), presents the pre-registered and exploratory findings side by side (§3.7), and adds three further analyses: a direct simulation of his stated peak-finding rule (script 11, §3.8.4) and a hemisphere-preserving permutation null with an asymmetry circularity diagnostic (scripts 12–13, §3.8.6), each specified and pre-committed in the analysis log before being run. These address his remaining testable objections; the points that cannot be resolved from available evidence (cross-site chronology) are disclosed as such. The analytic conclusion is unchanged.
...
| Data owner's peak-finding rule (11) | §3.8 | Exploratory | (0.96) / 0.75 / 0.34 | Rule reproduces his poles; not above chance under realistic nulls. Uniform value parenthesised: non-diagnostic of his binomial (see script 11b) |
| Per-degree binomial (11b) | §3.8 | Exploratory | uniform: 1.6e-9; cond: 0.030 | Baseline diagnosis: p collapses from 10⁻⁹ to 0.03 when baseline accounts for geometry |
| Hemisphere-preserving null (12) | §3.8 | Exploratory | III p_LEE: 0.014 site → 1.0 cluster | Preserves East/West asymmetry; Pole II dissolves (0.46); Pole III survives at site level, dissolves once replication removed |
| Asymmetry circularity check (13) | §3.8 | Exploratory | R = 0.96 | Hemispheric asymmetry ~96% reproduced from latitude clustering + geometry alone; the site-level survival is circular |
...
### Direct simulation of the data owner's stated peak-finding rule (post-publication)

A further objection raised in the data owner's 5 June 2026 letter (preserved as Appendix D below) was that the latitude look-elsewhere correction in script 07 modelled a procedure he did not use — that his actual method applied rule-based peak-finding criteria from a rules document shared before the database was opened, and that those rules impose their own multiplicity constraint not equivalent to a continuous-axis scan. Script 11 (`analysis/11_data_owner_rule_simulation.py`) addresses this directly by implementing his rules as stated (minimum 12 structures per degree, clusters extending over at least 3 degrees, gaps between clusters indicating discrete poles) and applying that procedure under three null models.

His rule, implemented faithfully, recovers his poles II–V on the observed data: it identifies two latitude clusters at 50–55°N (covering Pole V) and 60–85°N (covering Poles I through IV), and the per-cluster coverage of his claimed pole positions is Pole II = yes, III = yes, IV = yes, V = yes (Pole I = no). On this test his procedure performs as he describes; the data owner's characterisation of the §3.8.1 latitude look-elsewhere control as a "caricature" of his method is set aside.

The substantive question is whether his rule, applied to data produced by realistic nulls, finds clusters at his pole latitudes more often than chance. It does not. Under the conditional null with hemisphere preservation, the rule identifies 2.06 latitude clusters on average per replicate against the observed 2; under the block-conditional null, 1.38 clusters on average — both producing p(n_poles ≥ observed) > 0.33. Under the assumption-free conditional null, a qualifying cluster covered the 72°N band in 100% of random replicates (10,000 of 10,000). His rule finds poles at his claimed latitudes essentially always in random data, not because the data contain poles but because the rule's parameters guarantee a cluster will be found in any geographically realistic distribution. The rule's output on the observed data is reproduced; under realistic nulls that output is not distinguishable from its output on random data.

A second component of his published methodology — the per-degree binomial that produces the "100% probability" and "99.999% probability" claims — is also tested in script 11b. The binomial compares the observed count at each pole's degree (e.g., the observed count in the 72°N degree-bin, 36 structures) against a uniform expectation of ~11 structures per degree. Under that uniform baseline the p-values are astronomical: p(Pole I) = 3.7 × 10⁻⁵¹, p(Pole II) = 5.7 × 10⁻⁸, p(Pole III) = 1.6 × 10⁻⁹. When the identical binomial test is run with the per-degree expectation that the actual site geography produces under the conditional null (a non-uniform expectation reflecting that random great-circle intersections of mid-latitude sites pile in specific latitude bands by geometry), the p-values collapse to p(II) = 0.119, p(III) = 0.030, p(IV) = 0.077, p(V) = 0.324, and Pole I becomes p = 0.672. The small uniform-baseline p-values measure departure from a uniform sky, which is real and unremarkable; they do not measure clustering at the proposed poles. The remaining marginally significant cell — Pole III at p = 0.030 under the conditional null — is unremarkable in isolation and not significant after multiplicity correction across five poles.

This closes point 2 of the data owner's letter. His rule is faithful as published and his binomial is computed correctly, but the published confidence rests on a baseline that assumes structures distribute uniformly across the latitude axis when in fact the geography of the database produces specific non-uniform latitude distributions through pure geometry. The disagreement was never the rule; it was the baseline. Note that this script does not address point 3 of his letter — the question of whether the spatial-cluster null collapses genuinely independent structures (built by different cultures at different periods that happened to share an orientation convention) into one observation. That question remains empirically open pending chronological evidence for the structures in the 25 km spatial clusters of §3.8.3, particularly the 29-site cluster centred near 20.3°N, 89.6°W in the northern Yucatán. Pending that evidence, the v3 conclusion of indistinguishability from chance stands under the explicit assumption that the cluster null's collapse of spatially proximate structures is empirically justified for the dominant clusters; this assumption is testable and is being addressed (see correspondence with the data owner regarding the chronology of the dominant cluster).

### Synthesis (scripts 07–11)

The two controls resolve the discrepancy noted in §3.8.1. The assumption-free global conditional null saw no concentration (p_III = 0.90); the block-conditional null returned significance; the spatial-cluster null — which removes the independence assumption directly rather than approximating it through geographic blocks — confirms the global null. The block-conditional significance was manufactured by treating spatially autocorrelated structures as independent, and it erodes monotonically as that assumption is relaxed (coarse blocks → finer blocks → spatial clusters). With both the latitude look-elsewhere correction and a control for spatial non-independence applied, no proposed pole — including Poles II and III — shows clustering distinguishable from chance. This withdraws the central positive finding of version 2. It does not bear on the framework's geophysical interpretation, which an orientation-clustering analysis cannot test in any case (§4); it removes the statistical clustering that had been offered as that interpretation's evidence. A further objection — that these nulls do not preserve the data owner's hemispheric bearing asymmetry — is addressed in §3.8.6, which finds that asymmetry to be a geometric consequence of the latitude clustering rather than independent evidence for a pole.

### Hemisphere-preserving null and the circularity of the hemispheric asymmetry (scripts 12–13, post-publication)

A further objection, raised in the data owner's commentary (Appendix A) and pressed in his 8 June 2026 letter (Appendix E), is that the null models above permute bearings without preserving the systematic East/West hemispheric bearing asymmetry — Western structures rotated clockwise of true north, Eastern counterclockwise — from which he derived the 47°W meridian, and that they therefore erase the structure that produces the peaks before testing it. Scripts 12 and 13 test this directly. Their specification, hemisphere cut, decision rule, and interpretation were pre-committed in the analysis log before either was run.
Script 12 re-runs the conditional, block-conditional, and spatial-cluster nulls with bearings permuted only within hemispheres, so the per-hemisphere bearing distribution — and hence the asymmetry — is preserved by construction. Two cuts were used: an Americas/Old-World cut at 30°W, matching the asymmetry the data owner describes, and the prime meridian as a sensitivity check; the cuts agree. Under the hemisphere-preserving conditional null with the latitude look-elsewhere correction, Pole II dissolves (p_LEE = 0.46). Pole III retains a site-level residual (p_LEE = 0.014) but dissolves completely once spatially proximate structures are also collapsed to one independent unit each (p_LEE = 1.0 under the hemisphere-preserving spatial-cluster null; per-pole p rising from 0.0001 to 0.19). The Pole III contributors are not a single-hemisphere population — they split 73 West / 46 East under the Americas/Old-World cut — so the surviving residual is the within-cluster replication identified in §3.8.3, not the hemispheric asymmetry.
Script 13 asks whether the asymmetry the hemisphere-preserving null protects is itself a geometric consequence of the latitude clustering. Because a bearing of exactly true north sends a structure's great circle through the pole, reaching any far-northern intersection latitude on the 47°W meridian requires a small deviation whose sign is fixed by the structure's longitude relative to that meridian. Reconstructing, for each structure, the minimum-deviation bearing that reproduces the observed distribution of intersection latitudes — with no hemispheric information supplied — recovers a hemispheric asymmetry of +32.7° against an observed +34.0°: a reproduction fraction R = 0.96 (0.97 under the prime-meridian cut, and the asymmetry is reproduced even from uniformly drawn target latitudes, R ≈ 1.14). The observed East/West asymmetry is therefore approximately 96% entailed by the latitude clustering plus great-circle geometry alone. Preserving it in the null cannot serve as evidence for a former pole: the Pole III residual that survives the hemisphere-preserving conditional null does so because that null preserves a feature which is itself a consequence of the clustering under test. The survival is circular. Mean discards per permutation: 16.5% (sites that could not reach the assigned target latitude within ±45°). The diagnosis remains decisive: the lower bound of the 5–95% interval (0.92) is still well above the 0.80 threshold.
Point 1 is thus answered three independent ways: the Pole III contributors are not single-hemisphere; preserving the asymmetry while also removing spatial replication dissolves the pole entirely; and the asymmetry itself is geometrically entailed by the latitude clustering. No proposed pole shows clustering distinguishable from chance under any null that preserves a genuine, independent feature of the data.
...
A further response from the data owner was received on 5 June 2026 after v3 was deposited. His letter is preserved verbatim as Appendix D. Of his five points, point 2 (that the latitude look-elsewhere correction tested a procedure he did not use) is addressed by script 11 (§3.8.4), which implements his stated peak-finding rule and tests it directly. Two findings result. First, his rule, while faithful to his published method, finds no more poles under realistic nulls than in randomly permuted data. Second, his per-degree binomial — the source of his published high-confidence p-values — yields those values only against a uniform baseline; recomputed with the per-degree expectation the geometry actually produces, the same p-values become unremarkable. Point 3 (that the spatial-cluster null collapses genuinely independent structures) remains empirically open pending chronological evidence for the structures in the dominant spatial clusters identified in §3.8.3. Points 1, 4, and 5 are addressed by the side-by-side presentation of pre-registered and exploratory results in the revised §3.7 and §4.

A second follow-up was received on 8 June 2026 (preserved verbatim as Appendix E). The data owner conceded that script 11 implements his peak-finding rule faithfully and that his published "100%"/"99.999%" binomial probabilities rest on an unsupported uniform baseline, which he undertook to revise. Of his three further points: point 1 (that the nulls do not preserve the hemispheric bearing asymmetry) is answered by scripts 12 and 13 (§3.8.6); point 2 (archaeological dates) is disclosed-but-unresolved under the standing model-independent-dating criterion, the within-site replication question being separately settled by the database itself; and point 3 (the Pole I null finding) reflects the geometric convergence of meridians at the pole rather than a failure of method, as set out in the Appendix E author's note. Per the stopping rule recorded in the analysis log, scripts 12 and 13 are the final analyses before this deposit.
...
# Appendix E: Second follow-up correspondence from the data owner (8 June 2026), with author's note

Author's note. On 8 June 2026 the data owner replied to the 6 June response with the letter reproduced verbatim below. It conceded two points: that script 11 (§3.8.4) implements his peak-finding rule faithfully, and that the published "100%" and "99.999%" binomial figures rest on a uniform baseline the data do not support, which he undertook to revise. It raised three further points.
Point 1 — that the null models do not preserve the East/West hemispheric bearing asymmetry — was answered by two pre-committed analyses, scripts 12 and 13 (§3.8.6). A null that preserves the asymmetry leaves no signal at Pole II and, at Pole III, only a site-level residual that dissolves once spatial replication is also removed; and the asymmetry itself is ~96% reproducible from the latitude distribution plus great-circle geometry alone (R = 0.96), so preserving it cannot constitute evidence for a pole.
Point 2 — that model-independent construction dates for the eight northern-Yucatán locations are unavailable at the required precision, both because many structures are unexcavated and because orientation-based dates would be circular against the framework — the author accepts as a statement of the evidentiary limit. Under the criterion fixed before this correspondence (only radiocarbon, ceramic-seriation, or epigraphic dates can bear on cross-site independence), the cross-site convergence question is therefore disclosed-but-unresolved, and the conclusion stands under its stated assumption (§3.8.3, §4.5). The within-site replication question is separately resolved by the database itself: the 29 entries are eight locations with multiple bearings each.
Point 3 — that the null finding at Pole I (the current geographic pole) is a calibration failure — rests on a misreading of the statistic. The test measures clustering in excess of the geometric baseline. The current pole is the latitude at which great circles from all meridians converge, so a high count there is the geometric expectation, not a signal; the observed count (95) matching the null expectation (≈95) means there is no excess above that baseline, not that the method is blind to it. A genuine signal at Pole I would require a count above the convergence baseline, which the data do not show.
Per the stopping rule pre-committed in the analysis log, scripts 12 and 13 are the final analyses before this deposit. No point raised bears on the statistical question that is not now answered or, in the case of cross-site chronology, disclosed as unresolved for lack of model-independent evidence. The letter follows without alteration.
...
Gherbi S-E. Independent pre-registered verification of paleopole orientation clustering in the Buildreps database. Zenodo. Version 2 deposited 1 June 2026 (DOI: [10.5281/zenodo.20474028](https://doi.org/10.5281/zenodo.20474028)); version 3 deposited 5 June 2026 (DOI: [10.5281/zenodo.20546301](https://doi.org/10.5281/zenodo.20546301)); version 3.1 (this version) deposited 8 June 2026 (DOI: [10.5281/zenodo.20593690](https://doi.org/10.5281/zenodo.20593690)). [Version 2 reported a preliminary positive result at Poles II and III; version 3 withdrew it following the post-publication controls of §3.8; version 3.1 adds the data owner's 5 and 8 June 2026 follow-ups (Appendices D and E) and the analyses responding to them (scripts 11–13). See Appendix B for the relationship between versions.]
